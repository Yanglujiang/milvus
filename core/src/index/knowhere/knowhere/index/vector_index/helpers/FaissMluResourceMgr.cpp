...
FaissMluResourceMgr&
FaissMluResourceMgr::GetInstance() {
    static FaissMluResourceMgr instance;
    return instance;
}

void
FaissMluResourceMgr::AllocateTempMem(ResPtr& resource, const int64_t device_id, const int64_t size) {
    if (size) {
        resource->mlu_res->setTempMemory(size);
    } else {
        auto search = devices_params_.find(device_id);
        if (search != devices_params_.end()) {
            resource->mlu_res->setTempMemory(search->second.temp_mem_size);
        }
...
void
FaissMluResourceMgr::InitDevice(int64_t device_id, int64_t temp_mem_size, int64_t res_num) {
    DeviceParams params;
    params.temp_mem_size = temp_mem_size;
    params.resource_num = res_num;

    devices_params_.emplace(device_id, params);
    LOG_KNOWHERE_DEBUG_ << "DEVICEID " << device_id << "temp_mem_size "
                        << temp_mem_size / MB << "MB, resource count " << res_num;
}

void
FaissMluResourceMgr::InitResource() {
    if (!initialized_) {
        ...
        if (!initialized_) {
            for (auto& device : devices_params_) {
                ...
                for (int64_t i = 0; i < device_param.resource_num; ++i) {
                    auto raw_resource = std::make_shared<knowhere::StandardMluResources>();

                    auto res_wrapper = std::make_shared<Resource>(raw_resource);
                    res_wrapper->mlu_res->initMluResource(device_id);
...
ResPtr
FaissMluResourceMgr::GetRes(const int64_t device_id, const int64_t alloc_size) {
    fiu_return_on("FaissMluResourceMgr.GetRes.ret_null", nullptr);
    InitResource();
    ...
    if (finder != idle_map_.end()) {
        ...
        LOG_KNOWHERE_ERROR_ << "MLU device " << device_id << " not initialized";
...
void
FaissMluResourceMgr::MoveToIdle(const int64_t device_id, const ResPtr& res) {
    auto finder = idle_map_.find(device_id);
    if (finder != idle_map_.end()) {
        auto& bq = finder->second;
        bq.Put(res);
