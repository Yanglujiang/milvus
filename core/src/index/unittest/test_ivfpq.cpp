...
class IVFPQTest : public DataGen,
 ...
 protected:
    ...
    SetUp() override {
        ...
        int dim = 128;  // data dimension
        int nb = 1000000; // number of libray vectors
        int nq = 2;     // number of query
...
TEST_P(IVFPQTest, ivfpq_basic_mlu) {
    ...
    // milvus::knowwhere::MLUIVFPQ::Train()
    // use fassis::Level1Quantizer::train_q1 and fassis::IndexIVFPQ::train_residual_o finally
    // train results: 1.centroids: M * dsub * ksub   2. codes
    ...
    // base_dataset = std::make_shared<Dataset>()
    // base_dataset->Set(): set data
    // base_dataset->Get(): get data by key in map(data_ = map<std::string, std::shared_ptr<Value>)
    // base_dataset->data(): return data_
    /* ----------------    print raw_data && query results    ---------------*/
    ...
    auto train_ids = base_dataset->Get<const int64_t*>(milvus::knowhere::meta::IDS);
    for(int i = 0; i < rows; i++){
        std::cout<<"----------- index is :"<<((int64_t*)train_ids)[i]<<" ------------"<<std::endl;
        std::cout<<"[";
        for(int j = 0; j < dim; j++){
            std::cout<<((float*)raw_data)[i * dim + j]<<",";
        }
        std::cout<<"]"<<std::endl;
    } */
    ...
    // index_->Query --> Milvus::knowwhere::IVF::Query() --> Milvus::knowwhere::MLUIVFPQ::QueryImpl() 
