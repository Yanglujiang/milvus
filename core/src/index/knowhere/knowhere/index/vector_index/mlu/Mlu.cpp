...
MluInterface::Search(int64_t num_query, const float *data, int64_t k, float *distance, int64_t *labels) {
    ...
    int64_t query_request = sizeof(float) * search_size * d + sizeof(float) * search_size * m * 128 * 9 
	    + sizeof(float) * search_size * nb + sizeof(float) * search_size * topk +sizeof(int32_t) * search_size * topk;
	...
	MluTopk(
        ...
        cnrtMemcpy(distance + query_loop * topk, (char *)query_dev + topk_out_offset,  sizeof(float) * search_size * topk, CNRT_MEM_TRANS_DIR_DEV2HOST);
