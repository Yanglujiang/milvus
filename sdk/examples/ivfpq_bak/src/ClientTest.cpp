...
constexpr int64_t SEARCH_TARGET = BATCH_ENTITY_COUNT;  // change this value, result is different
constexpr int64_t ADD_ENTITY_LOOP = 1;
constexpr int32_t NLIST = 1;
...
ClientTest::CreateCollection() {
    ...
    json_param = {{"auto_id", false}, {"segment_row_limit", 14096}};
...
template <typename T>
std::vector<T> Range(T start, T end, T stride)
{
        std::vector<T> ret;
...
ClientTest::InsertEntities() {
    //std::vector<int32_t> duration = Rand<int32_t>(BATCH_ENTITY_COUNT, 250, 300);//{208, 226, 252};
    //std::vector<int32_t> release_year= Rand<int32_t>(BATCH_ENTITY_COUNT, 2002, 2003);//{2001, 2002, 2003};

    std::vector<milvus::VectorData> embedding;
    milvus_sdk::Utils::BuildVectors(COLLECTION_DIMENSION, BATCH_ENTITY_COUNT, embedding);
    ...
    std::vector<int64_t> id_array = Range<int64_t>(1, BATCH_ENTITY_COUNT + 1, 1);//{1, 2, 3};
...
ClientTest::SearchEntities() {
    ...
    std::vector<std::string> get_fields{"embedding"};
...
ClientTest::CreateIndex(const std::string& collection_name, int64_t nlist) {
    ...
    JSON json_params = {{"index_type", "IVF_PQ"}, {"metric_type", "L2"}, {"params", {{"nlist", nlist}, {"m", 4}}}};
...
ClientTest::DropCollection(const std::string& collection_name) {
    std::cout << "DropCollection function Start: "<< std::endl;
