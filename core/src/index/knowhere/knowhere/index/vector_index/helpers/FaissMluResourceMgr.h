...
struct Resource {
    explicit Resource(std::shared_ptr<knowhere::StandardMluResources>& r) : mlu_res(r) {
        static int64_t global_id = 0;
        id = global_id++;
    }

    std::shared_ptr<knowhere::StandardMluResources> mlu_res;
    int64_t id;
    std::mutex mutex;
};
...
class FaissMluResourceMgr {
 ...
 public:
    static FaissMluResourceMgr&
    GetInstance();

    // Free mlu resource, avoid cudaGetDevice error when deallocate.
    ...
    void
    InitDevice(int64_t device_id, int64_t temp_mem_size = 0, int64_t res_num = 1);

    void
    InitResource();

    // allocate mlu memory invoke by build or copy_to_mlu
    ResPtr
...
class ResScope {
 public:
    ...
    // specif for search
    // get the ownership of mluresource and mlu
    ...
    Lock() {
        if (own)
            FaissMluResourceMgr::GetInstance().mutex_cache_[device_id]->lock();
        resource->mutex.lock();
    }

    ~ResScope() {
        if (own)
            FaissMluResourceMgr::GetInstance().mutex_cache_[device_id]->unlock();
        if (move)
            FaissMluResourceMgr::GetInstance().MoveToIdle(device_id, resource);
